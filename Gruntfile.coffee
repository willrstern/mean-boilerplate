module.exports = (grunt)->
  # Default to `grunt server`
  grunt.registerTask('default', [ 'server' ])

  # Run `grunt server` for live-reloading development environment
  grunt.registerTask('server', [ 'build', 'livereload-start', 'express', 'regarde' ])
  ...
  grunt.registerTask('build', [ 'clean', 'jshint', 'copy', 'ngtemplates', 'concat', 'less' ])
  ...
  grunt.config.init
    ...
    concat:
      admin:
        files:      [
          src:      '<%= CLIENT_DIR %>/admin/<%= JS_FILES %>'
          dest:     '<%= BUILD_DIR %>/admin/admin.combined.js'
        ]
    ...
    less:
      '<%= BUILD_DIR %>/admin/styles/admin.css': '<%= CLIENT_DIR %>/admin/styles/admin.less'
    ...
    ngtemplates:
      admin:
        cwd:        '<%= CLIENT_DIR %>'
        src:        'admin/<%= HTML_FILES %>'
        dest:       '<%= BUILD_DIR %>/admin/templates.js'
        options:
          concat:   'admin'
      components:
        cwd:        '<%= COMPONENTS_DIR %>/angular-ui-bootstrap'
        src:        [ 'template/accordion/<%= HTML_FILES %>'
                      'template/tabs/<%= HTML_FILES %>'
                      'template/tooltip/<%= HTML_FILES %>'
                      'template/typeahead/<%= HTML_FILES %>' ]
        dest:       '<%= BUILD_DIR %>/admin/ui.templates.js'
        options:
          module:   'admin'
          concat:   'admin'
